...
import { Users, FileText, TrendingUp, Eye, Sparkles, Award, Download, Info, ArrowUpRight, ArrowDownRight } from "lucide-react";
...
const PLATFORM_COLORS: Record<string, string> = {
  ...
  tiktok: "#ffffff", linkedin: "#0A66C2", twitter: "#8899aa",
};
...
const generateFollowerGrowth = () => {
  ...
  return Array.from({ length: days }, (_, i) => {
    ...
    return { date: date.toLocaleDateString("en-US", { month: "short", day: "numeric" }), instagram: 12000 + Math.floor(Math.random() * 800 + i * 30), facebook: 8500 + Math.floor(Math.random() * 500 + i * 20), youtube: 5200 + Math.floor(Math.random() * 400 + i * 25), linkedin: 3100 + Math.floor(Math.random() * 300 + i * 15), tiktok: 9800 + Math.floor(Math.random() * 600 + i * 40), twitter: 4500 + Math.floor(Math.random() * 350 + i * 10) };
  });
};

const generatePostingFrequency = () => ["Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"].map((d) => ({ day: d, instagram: Math.floor(Math.random() * 4 + 1), facebook: Math.floor(Math.random() * 3 + 1), youtube: Math.floor(Math.random() * 2), tiktok: Math.floor(Math.random() * 3 + 1), linkedin: Math.floor(Math.random() * 2 + 1), twitter: Math.floor(Math.random() * 5 + 2) }));

const generateEngagement = () => Object.entries(PLATFORM_COLORS).map(([key, color]) => ({ platform: PLATFORMS.find((p) => p.dbKey === key)?.name || key, rate: +(Math.random() * 5 + 1).toFixed(2), color }));

const generateHeatmap = () => ["Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"].map((day) => ({ day, hours: Array.from({ length: 15 }, (_, i) => ({ hour: i + 8, value: Math.random() })) }));
...
const AnalyticsPage = () => {
  ...
  const visiblePlatforms = platformFilter === "all" ? Object.keys(PLATFORM_COLORS) : [platformFilter];

  return (
    <div className="animate-fade-in space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-[28px] font-display font-bold text-foreground">Analytics</h1>
          {selectedCompany && <p className="text-sm text-muted-foreground">{selectedCompany.name}</p>}
        </div>
        <div className="flex items-center gap-3">
          <Select value={range} onValueChange={setRange}><SelectTrigger className="w-[150px]"><SelectValue /></SelectTrigger><SelectContent>{RANGE_OPTIONS.map((r) => <SelectItem key={r.value} value={r.value}>{r.label}</SelectItem>)}</SelectContent></Select>
          <Select value={platformFilter} onValueChange={setPlatformFilter}><SelectTrigger className="w-[150px]"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="all">All Platforms</SelectItem>{PLATFORMS.slice(0, 6).map((p) => <SelectItem key={p.dbKey} value={p.dbKey}>{p.name}</SelectItem>)}</SelectContent></Select>
      ...
      <div className="grid grid-cols-2 gap-4 md:grid-cols-3 lg:grid-cols-6">
        {metrics.map((m) => (
          <div key={m.label} className="card-glass shimmer-card p-4 transition-all duration-300 hover:-translate-y-1">
            ...
            <p className="mt-2 text-2xl font-mono-stat font-bold gradient-text relative z-10">{m.value}</p>
            <p className="text-xs text-muted-foreground relative z-10">{m.label}</p>
          </div>
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
        ...
        <Card><CardHeader className="pb-2"><CardTitle>Best Posting Times</CardTitle></CardHeader><CardContent>
          <div className="space-y-1">
            <div className="flex gap-1 ml-10">{Array.from({ length: 15 }, (_, i) => i + 8).map((h) => (<div key={h} className="flex-1 text-center text-[9px] text-muted-foreground">{h > 12 ? `${h - 12}p` : h === 12 ? "12p" : `${h}a`}</div>))}</div>
            {heatmapData.map((row) => (
      ...
      <Card><CardHeader className="pb-2"><CardTitle>Content Performance — Top Posts</CardTitle></CardHeader><CardContent>
          ...
          {topPosts.map((post) => {
            ...
            return (
              <div key={post.id} className="flex items-center gap-4 py-3">
                <div className="flex h-10 w-10 items-center justify-center rounded-lg shrink-0" style={{ backgroundColor: `${PLATFORM_COLORS[post.platform]}20` }}>{platform && <platform.icon className="h-5 w-5" style={{ color: PLATFORM_COLORS[post.platform] }} />}</div>
                <div className="flex-1 min-w-0"><p className="text-sm font-medium truncate text-foreground">{post.caption}</p><p className="text-xs text-muted-foreground font-mono-stat">{post.date}</p></div>
                <div className="hidden sm:flex items-center gap-6 text-sm text-muted-foreground"><span>❤ {post.likes.toLocaleString()}</span><span>💬 {post.comments}</span><span>👁 {post.reach.toLocaleString()}</span></div>
              </div>
            );
          })}
        </div>
      ...
      <div>
        <h2 className="mb-4 text-lg font-display font-semibold text-foreground">Platform Breakdown</h2>
            ...
            return (
              <Card key={stat.key}>
                <CardContent className="p-4">
                  <div className="flex items-center gap-3">
                    <div className="flex h-10 w-10 items-center justify-center rounded-lg" style={{ backgroundColor: `${PLATFORM_COLORS[stat.key]}20` }}><platform.icon className="h-5 w-5" style={{ color: PLATFORM_COLORS[stat.key] }} /></div>
                    <div><p className="font-display font-medium text-foreground">{platform.name}</p><p className="text-xs text-muted-foreground">{stat.followers} followers</p></div>
                    <span className={`ml-auto flex items-center text-xs font-medium ${stat.growth >= 0 ? "text-success" : "text-destructive"}`}>{stat.growth >= 0 ? <ArrowUpRight className="h-3 w-3" /> : <ArrowDownRight className="h-3 w-3" />}{Math.abs(stat.growth)}%</span>
                  </div>
                  <div className="mt-3 grid grid-cols-2 gap-3">
                    <div><p className="text-xs text-muted-foreground">Posts</p><p className="text-lg font-mono-stat font-semibold text-foreground">{stat.posts}</p></div>
                    <div><p className="text-xs text-muted-foreground">Engagement</p><p className="text-lg font-mono-stat font-semibold text-foreground">{stat.engagement}%</p></div>
                  </div>
